  ...
  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="w-full max-w-md mx-auto p-0 rounded-t-3xl border-0 bg-[#F7F9FB] max-h-[95vh] overflow-y-auto">
                    ...
                    {cartItems.map(({ item, quantity }) => (
                        <div key={item.id} className="flex items-center p-3 bg-white rounded-2xl shadow-sm border border-gray-100/80">
                            ...
                            <div className="flex-1 px-4">
                                ...
                                <p className="font-extrabold text-teal-600 text-base mt-1">AED {item.price.toFixed(2)}</p>
                            </div>
                ...
                <div className="space-y-4 pt-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {upsellItems.map(item => (
                            <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-3 text-center">
                                <Image src={item.image} alt={item.name} width={150} height={100} className="w-full h-24 object-cover rounded-xl" />
                                <h4 className="font-bold text-sm text-gray-800 mt-3 truncate">{item.name}</h4>
        ...
        {/* Footer */}
        <div className="p-4 bg-[#F7F9FB] border-t border-gray-200/80 shadow-[0_-4px_12px_rgba(0,0,0,0.03)] space-y-4">
